...
if printdocs
    ...
    [doctxt] = textscan(fopen(strcat(dir,'/papers.csv')), '%s','Delimiter','\n');
    doctxt = doctxt{1};
...
repTopics = fopen(strcat(save,'/topic_words.txt'),'w');
...
for ik = 1:K
   ...
   fprintf(repTopics, '\n');
    ...
    for i = 1:10
        if printdocs
            doc_str = doctxt(idx(i));
            x = strsplit(doc_str{1});
            tw = x(1);
            sl = size(tw{1},2);
            if sl>600 
                sl = 600;
